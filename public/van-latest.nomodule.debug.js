(() => {
  ...
  var updateDoms = () => {
    ...
    for (let s of changedStatesArray)
      s._oldVal = s.rawVal;
    renderResolver?.(), renderResolver = _undefined;
  };
  var hydrate = (dom, f) => update(dom, bind(f, dom));
  var van_default = { add, tags, state, derive, hydrate, render: () => new Promise((r) => renderResolver = r) };
  ...
  var _tags = _tagsNS("");
  var tags2 = new Proxy(tagsNS, { get: (_, name) => _tags[name] });
  var hydrate2 = (dom, f) => {
  ...
  var van_debug_default = { add: add2, tags: tags2, state: state2, derive: derive2, hydrate: hydrate2, render: van_default.render, startCapturingErrors, stopCapturingErrors, get capturedErrors() {
