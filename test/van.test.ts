...
const runTests = async (van: VanForTesting, msgDom: Element, {debug}: BundleOptions) => {
  ...
  const assertEq = (
    lhs: string | number | Node | undefined, rhs: string | number | Node | undefined) => {
  ...
  const waitForRender = async () => await Promise.race([
    van.render(),
    sleep(5).then(() => timeout)
  ])
  ...
  const tests = {
    ...
    tags_stateAsProp_connected: withHiddenDom(async hiddenDom => {
      ...
      assert(await waitForRender() !== timeout)
    ...
    derive_sideEffect: async () => {
      ...
      s.val = "test"
      // `s.val` isn't changed thus the side effect won't be triggered
      assert(await waitForRender() === timeout)
      assertEq(JSON.stringify(history), '["This","is","a","test"]')
  ...
  const hydrateExample = (
    ...
    const clickBtns = async (dom: HTMLElement, numUp: number, numDown: number, reactive: boolean) => {
      const [upBtn, downBtn] = [...dom.querySelectorAll("button")]
      for (let i = 0; i < numUp; ++i) {
        upBtn.click()
        reactive ? assert(await waitForRender() !== timeout) : assert(await waitForRender() === timeout)
      }
    ...
    await clickBtns(basicCounter, 3, 1, false)
    await clickBtns(styledCounter, 2, 5, false)
    ...
    // A new rendering cycle won't be triggered in `examples.hydrate`, but will be triggered in
    // `examples.hydrateOptimized` due to the internal details of the implementation.
    await waitForRender()
    ;[basicCounter, styledCounter] = hiddenDom.querySelectorAll("div")
    assertEq(styledCounter.innerHTML, counterHTML(5, "👆👇"))
    selectDom.value = "👆👇"
    ...
    ;[basicCounter, styledCounter] = hiddenDom.querySelectorAll("div")
    await clickBtns(basicCounter, 3, 1, true)
    await clickBtns(styledCounter, 2, 5, true)
  ...
  const gcTests = {
    ...
    long_conditionalBinding: withHiddenDom(async hiddenDom => {
      ...
      for (let i = 0; i < 100; ++i) {
        ...
        // Sometimes a new rendering cycle will be triggered. Sometimes it won't be triggered.
        await waitForRender()
      }
